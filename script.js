...
var timerBlock = document.querySelector("#timer"); // обращаемся внутри функции к блоку timer // YOU NEED TO DEFINE TIMERBLOCK IN GLOBAL SCOPE AS YOU TRY TO REACH IT IN DIFFERENT FUNCTIONS

var CHASY; //YOU NEED TO DEFINE TIMER ID IN THE GLOBAL SCOPE IF YOU WANT TO ACCESS IT IN DIFFERENT FUNCTIONS
...
// YOU DON'T NEED HERE NEITHER SETINTERVAL NO SET TIMEOUT
    stars.innerText = i = 0; //счетчик текста = 0 сброс счетчика
    timerBlock.innerText = 0;// при окончании игры обнуляет счетчик id timmer //NO VARIABLE TIMER
...
function startIgra() { //создаем имя функции для изменения стилей на стороне клиента старта игры
    timerBlock.innerText = 10; // WHEN RESTART GAME YOU WANT TO SET TIMER BACK TO 10 SECONDS
    ...
    CHASY = setInterval(function nes() { //присваимаем всей пременной chasy действие отсчета через 1 сек
        // var timerBlock = document.querySelector("#timer"); // Присваиваем переменную id timer с html //YOU NEED TO DEFINE TIMERBLOCK ONCE IN GLOBAL SCOPE AND YOU HAVE ACCESS TO IT IN DIFFERENT FUNCTIONS
